...
def check_role_and_relationship_to_corporation(role_admin_id=0):
    def decorator_admin(func):
        def check_func(corporation_id, *args, **kwargs):
            admin = current_user.admins.filter(
                ...
                Admin.role_admin_id < role_admin_id).first()
            if admin:
...
def check_role_and_relationship_to_company(role_employee_id=0):
    def decorator_employee(func):
        def check_func(company_id, *args, **kwargs):
            employee = current_user.employees.filter(
                Employee.company_id == company_id,
                Employee.role_employee_id < role_employee_id).first()
